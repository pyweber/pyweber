from pyweber.utils.types import WindowEvents, WindowEventType
...
class Window:
    ...
    def __repr__(self):
        ...
        return (
            ...
            f"location={self.location}, navigator={self.navigator})"
        )
    ...
    def add_event(self, event_type: WindowEventType, event: WindowEvents):
    ...
    def open(self, url: str, target: str = "_blank", features: str = "") -> "Window":
        ...
        return Window()  # Retorna uma nova instância de Window
    ...
    def set_interval(self, callback: callable, interval: int):
        ...
        def interval_callback():
            callback()
            threading.Timer(interval / 1000, interval_callback).start()
        interval_callback()
