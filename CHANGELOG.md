...
- **Safe redirects** — `Window.open` / `to_url` / `launch_url` allow relative `/…` or hosts in `allowed_redirect_hosts` / `PYWEBER_ALLOWED_REDIRECT_HOSTS`.
- **JS-injected DOM sync** — debounced `MutationObserver` + `window.__pyweber_adopt(el)` / `__pyweber_resyncDom()` stamp uuids and `merge_client_dom` grafts into the session; disable with `<meta name="pyweber-dom-watch" content="off">`.
...
- **WS DOM sync** — client outerHTML is **merged by uuid** into the existing Python tree (no wholesale `parse_html` replace). Handoff **moves** the same `Template` instance into the session. `Element`/`Template.clone` preserve subclass type and rebind `self.handler` / Element refs; route `Element`s are adopted without HTML round-trip. Bound handlers can use `self` after the first render. See [docs/guides/reactivity.md](docs/guides/reactivity.md).
...
- **EventBook lifecycle** — cleanup by element uuid when a session is removed.
- **WS `close`/`send`** — no discarded coroutines from `remove_connection` / `send_all`.
- **`wsMessage.template`** — lazy `ensure_template()` (no un-awaited coroutine in `__init__`).
- **`JWTAlgorithms`** — public import warns; enum-only until **2.0**.
...
- **Reactive WebSocket events** — end-to-end fixes so clicks / `e.update()` work reliably:
  ...
  - **Form `values` on clicks** — `insert_values` always runs when the payload has `values`, even with `template: null`.
  - **`e.update()` from sync handlers** — schedule `send_message` on the WS event loop via `run_coroutine_threadsafe`.
  - **Client `setSessionId`** — always accept the server id; apply it before flushing buffered window events.
- **`<script>` / `<style>` bodies** are not HTML-escaped under `sanitize=True` (also tracked for 1.5.2 line).

### Removed

- **`Element.update()`** stub — use `e.update()`; surgical per-element WS push remains out of scope.
